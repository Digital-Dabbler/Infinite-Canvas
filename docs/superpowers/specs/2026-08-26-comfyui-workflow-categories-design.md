...
不再在 `workflows/` 根目录放置任一类别的工作流 JSON。工作流 API 通过明确的 `source` 字段表达来源，不靠名称、文件图标或当前硬编码内置清单推断。
...
- `GET /api/workflows` 返回每项的 `source`，并支持按来源筛选；现有调用在未提供筛选时仍能获得完整列表，避免其他管理界面立即失效。
...
- 既有画布中保存的根目录系统工作流名会在读取/运行时映射到 `system/<name>`。迁移完成后，根目录不再作为正常读取来源。
...
- 固定功能工作流缺失或映射不存在时，阻止提交并返回明确的“系统工作流不可用”错误，禁止退回到列表第一个自定义工作流。
- 自定义工作流不存在、已删除或字段映射不合法时，保持现有明确错误响应，不影响系统功能工作流。
...
- 角度生成请求确实引用 `system/comfyui-workflow-multiple-angles-api.json`，且结果面板显示“多角度参考图生成”。
- 自定义选择器不显示系统工作流。
- 旧画布中保存的根目录工作流名在迁移后仍可加载和运行，且不会误选其他工作流。
